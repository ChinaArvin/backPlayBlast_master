...
def Creat_camera():
    ...
    camera_x = aim_x + 8 * radiousValue
    camera_y = aim_y + 8 * radiousValue
    camera_z = 0

    # cam_1 = cmds.camera(name='cam_1')
    cam_2 = pm.camera(name='cam_1')
    pm.lookThru(cam_2)
    cams = cmds.ls(type='camera')
    for cam in cams:
        cmds.setAttr(cam + '.rnd', 0)
    cmds.setAttr('cam_1Shape1.rnd', 1)
    cams = cmds.ls(type='camera')
    for cam in cams:
        cmds.setAttr(cam + '.rnd', 1)

    cmds.move(camera_x, camera_y, camera_z)
    cmds.rotate(-40, 90, 'cam_1')
...
def TakeScreen(path,publishPath=""):
    ...
    movPath = "%s/%s" % (videosPath, videosName)
    print movPath
    cmds.playblast(filename=movPath, format="avi", sequenceTime=0,
                   ...
                   quality=60, widthHeight=[960, 720])
