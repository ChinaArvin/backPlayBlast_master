...
class writePathPannel(QtGui.QDialog):
    def __init__(self,n,parent = None):
        ...
        self.sequence_line.setEnabled(False)
        self.shot_line = QtGui.QLineEdit()
        self.shot_line.setEnabled(False)
        self.tasktype_line = QtGui.QLineEdit()
        self.tasktype_line.setEnabled(False)
    ...
    def combChange(self):
        ...
        if self.filetype_combbox.currentText() == "mov":
            print "111"
            self.info['filename'] = os.path.basename(self.rootname).replace('.nk','.mov')
