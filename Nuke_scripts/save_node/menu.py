...
TOOLKIT_ROOT = 'E:/pycharm_Code/Nuke_scripts/toolkit'
...
def add_user_template(menu, root_folder, template=None):
    ...
    for folder in folders:
        ...
        if templates:
            ...
            for template in templates:
                template_name = copy.deepcopy(os.path.basename(template).split('.')[0])  #test1 test2
                submenu.addCommand('{}/{}'.format(template_name, template_name),
                                   lambda template = template: nuke.scriptReadFile('{}/{}/{}'.format(root_folder, folder, template)))
