...
class ElementLoaderView(QtGui.QWidget):
    ...
    def set_version(self,row_count,versions,task,sequence,shot):
        ...
        for index in range(row_count+1):
            ...
            # self.version_combo.set_value(row=index, task=task)
            for t in versions:
            ...
            # print "path:", sequence, shot, task, current_version
            # resolutions = self.search_resolution(sequence, shot, task, current_version)
    ...
    def set_resolution(self,info):
        ...
        self.shot = info['shot']
        resolutions = self.search_resolution(self.sequence, self.shot, self.task,self.version)
        index =info['row']
