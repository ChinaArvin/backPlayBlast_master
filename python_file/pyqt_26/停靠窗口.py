...
class MainWidget(QMainWindow):
    def __init__(self,parent = None):
        ...
        #停靠窗口1
        dock1 = QDockWidget(self.tr("依靠窗口 1"),self)
        dock1.setFeatures(QDockWidget.DockWidgetMovable)
        dock1.setAllowedAreas(Qt.LeftDockWidgetArea|Qt.RightDockWidgetArea)
        te1 = QTextEdit(self.tr("窗口1,可在MainWidget的左部和右部停靠,不可浮动,不可关闭"))
        dock1.setWidget(te1)
        self.addDockWidget(Qt.RightDockWidgetArea,dock1)

        #停靠窗口2
        dock2 = QDockWidget(self.tr("依靠窗口2"),self)
        ...
        te3 = QTextEdit(self.tr("窗口3,可在MainWidget任意位置移动,可浮动,可关闭"))
        dock3.setWidget(te3)
