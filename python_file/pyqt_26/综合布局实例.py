...
class StockDialog(QDialog):
    def __init__(self,parent = None):
        ...
        mainSplitter.setOpaqueResize(True)            #用由设定分割窗的分割条在拖动时是否为 实时更新显示
        ...
        amentPushButton = QPushButton(self.tr("修改"))
        closePushButton = QPushButton(self.tr("关闭"))

        buttonLayout = QHBoxLayout()
        buttonLayout.addStretch(1)                       #该函数的作用就是平分布局，它所带的参数就是所占的比例
        buttonLayout.addWidget(amentPushButton)
...
class BaseInfo(QWidget):
    def __init__(self,parent = None):
        ...
        iconLabel = QLabel()
        icon = QPixmap("E:\pycham\lovely.png")
